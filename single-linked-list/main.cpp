...
// The supporting class in which the copy constructor and assignment operator are removed
class NoCopy{
public:
    NoCopy() = default;

    NoCopy(int* data) : data_{data} {}
...
// Test move constructor and methods
void Test5() {
    SingleLinkedList<NoCopy> list;

    int value = 10;
    int* p_value = new int{5};
    NoCopy no_copy{&value};

    list.PushFront(std::move(no_copy));
    auto pos = list.InsertAfter(list.begin(), NoCopy(p_value));

    assert(list.begin()->Get() == &value);
    assert(pos->Get() == p_value);

    SingleLinkedList<NoCopy> list2(std::move(list));
    pos = ++list2.begin();

    assert(list2.begin()->Get() == &value);
    assert(pos->Get() == p_value);
